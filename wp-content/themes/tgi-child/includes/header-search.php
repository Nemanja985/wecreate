		     	...
		     	<div class="inner-wrap">
			     	...
			     	<div class="col span_12">
				      	<form action="<?php echo esc_url( home_url( '/' ) ); ?>" method="GET">
				      		...
				      		<?php } else { ?> 
				      			<input type="text" name="s" <?php if($ajax_search == 'yes') { echo 'id="s"'; } ?> value="<?php // echo __('Search', NECTAR_THEME_NAME); ?>" data-placeholder="<?php echo __('Start Typing...', NECTAR_THEME_NAME); ?>" />
				      		<?php } ?>
				      	</form>
								
								<?php 
							echo '<span>' . __('Hit enter to search or ESC to close', NECTAR_THEME_NAME) . '</span>';
								// if($theme_skin == 'ascend' && $ajax_search == 'no') { echo '<span><i>'.__("Press enter to begin your search",NECTAR_THEME_NAME).'</i></span>'; }
